...
> When Eugene assesses an AI system, it uses BOTH frameworks.
...
| AI RMF Subcategory | Gap vs 800-53 | Why It Matters for Eugene |
|---|---|---|
| MEASURE 2.5 (output trustworthiness) | 800-53 doesn't assess LLM output quality | Eugene's findings must be factually grounded |
| MEASURE 2.9 (fairness/bias) | 800-53 doesn't measure equitable treatment | Eugene should classify risk consistently across system types |
| MEASURE 2.10 (privacy) | 800-53 PT family is partial | Eugene processes scanner output — check for PII in raw fields |
| MEASURE 2.11 (security/resilience) | 800-53 misses AI-specific attacks | Prompt injection against Eugene could corrupt findings |
| MAP 3.2 (stakeholder impacts) | 800-53 doesn't require AI-specific stakeholder analysis | Who is harmed if Eugene produces wrong findings? |
| GOVERN 1.5 (AI risk tolerance) | 800-53 RA-3 is general | AI systems need explicit tolerance statements ("Eugene can err on noisy C-rank, never on B/S") |

---

## Finding Classification Guide

When Eugene produces a finding, classify it using this guide:
...
## Eugene System Prompt Addition (AI RMF context)

When Eugene encounters an AI-related finding, it adds this to its output:
...
This dual citation is what distinguishes a Eugene finding from a standard scanner report.
