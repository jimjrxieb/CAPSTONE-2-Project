...
| CAP2-AI-001 | Eugene | AI security assessment assistant for RAG and AI adoption reviews | Local LLM via Ollama | v0.1 build target | Limited Risk | In Build | 2026-06-08 | 2026-06-08 |
...
| **Data Processed** | Synthetic MedData Nexus documents, RAG retrieval context, scenario evidence, framework mappings |
| **HITL Enforced** | Yes — HITL router required before any B/S output |
| **Serving** | Planned: local API `/api/eugene`; implementation tracked in `CBBP-PLAN/BUILD/rag-pipeline-build.md` |
| **Monitoring** | Planned: evidence capture under `evidence/` plus optional MLflow tracking |
| **Known Limitations** | Advisory only; not an autonomous assessor; final judgment remains human-owned |
| **Registration Link** | `CBBP-PLAN/BUILD/rag-pipeline-build.md` |
...
|---|---|---|---|
| 2026-06-08 | Eugene registered as Capstone 2 build target | jimjrxieb | GOVERN 1.1 |
...
2. **GOVERN 1.1 FAIL** — Eugene will generate a finding
...
kubectl get deployments -A -o json | jq '.items[] | select(.metadata.name | test("ai|llm|model|ollama|jade|katie|eugene"; "i")) | {ns: .metadata.namespace, name: .metadata.name}'

# Find AI-related FastAPI routes in this repo
grep -r "api/jade\|api/katie\|api/eugene\|api/ai\|api/llm" . --include="*.py"
```

Compare against this register. Any system found but not registered → Eugene generates GOVERN 1.1 FAIL finding.
